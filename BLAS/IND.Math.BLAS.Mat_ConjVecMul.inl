  ...
  T_Vec_y y_, Stride y_s )
{
  ...
  const auto A_rs = Lyt::RowStride( A_, A_ld );
  const auto A_cs = Lyt::ColStride( A_, A_ld );
  ...
  case Trnsp::Yes:
    {
      ...
      {
        const auto axi = alpha*Conj( x(i) );
        const auto A_row = Lyt::ColPtr( A_, i, 0, A_ld );
